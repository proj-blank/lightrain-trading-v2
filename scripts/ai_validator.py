...
def validate_position(ticker, entry_price, technical_score, rs_rating, indicators_fired, df):
    ...
    try:
        # Ensure entry_price is float
        entry_price = float(entry_price)

        # Extract recent price action and convert to floats
        if not df.empty and "Close" in df.columns:
            recent_prices = [float(p) for p in df["Close"].tail(5)]
        else:
            recent_prices = [entry_price] * 5

        if not df.empty and "Volume" in df.columns:
            recent_volumes = [float(v) for v in df["Volume"].tail(5)]
        else:
            recent_volumes = []
